...
| `claim_status` | enum | `OPEN` \| `CLOSED` \| `REOPENED` \| `UNKNOWN` |
...
**Stage 7 — Export** (`core/export.py`) — Excel workbook, three sheets: `Claims`, `Exceptions`, `Source Info` (filename, hash, valuation date, extraction method, timestamp, reconciliation status).
...
| R-11 | Duplicate `claim_number` within one policy period | WARN |
...
**Metric:** field-level accuracy = correct non-null fields / total expected non-null fields. Report separately for money fields and text fields — money accuracy is the number that matters.
