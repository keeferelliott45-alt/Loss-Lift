...
def test_claims_sheet_has_a_row_per_claim(extracted):
    sheet = build_workbook(extracted.document, extracted.reconciliation)["Claims"]
...
def test_exceptions_sheet_says_so_when_clean(golden_dir):
    ...
    assert "No exceptions" in sheet["A2"].value
...
def test_every_template_builds(extracted, template):
    ...
    assert len(headers) == len(resolve_columns(template))
...
def test_custom_column_order():
    ...
    assert [cell.value for cell in workbook["Claims"][1]] == ["Total incurred", "Claim number"]
...
def test_export_survives_an_empty_document():
    ...
    assert workbook["Claims"].max_row == 1
